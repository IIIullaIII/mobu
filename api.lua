...
local peaceful_only = minetest.setting_getbool("only_peaceful_mobu")
local disable_blood = minetest.setting_getbool("mobu_disable_blood")

mobu.protected = tonumber(minetest.setting_get("mobu_spawn_protected")) or 1
mobu.remove = minetest.setting_getbool("remove_far_mobu")
...
function mobu:register_mob(name, def)
...
minetest.register_entity(name, {
	...
	blood_texture = def.blood_texture or "mobu_r_blood.png",
		...
		if self.attack_type == "explode" then
			...
			if dist > self.reach then
				...
				if self.timer > 3 then
					...
					-- hurt player/mobu caught in blast area
					...
					mobu:explosion(pos, 2, 0, 1, self.sounds.explode)
	...
	new_on_punch = function(self, hitter, tflp, tool_capabilities, dir)
		...
		-- attack puncher and call other mobu for help
...
end -- END mobu:register_mob function

-- global functions

mobu.spawning_mobu = {}

function mobu:spawn_specific(name, nodes, neighbors, min_light, max_light,
	interval, chance, active_object_count, min_height, max_height)

	mobu.spawning_mobu[name] = true
	...
	if new_chance ~= nil then
		chance = new_chance
		print ("[mobu Redo] Chance setting for " .. name .. " is now " .. chance)
	end
	...
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not mobu.spawning_mobu[name] then
				return
			end
			...
			-- mobu cannot spawn in protected areas when enabled
			if mobu.protected == 1
			...
			if mob == false then
				print ("[mobu]" .. name .. " failed to spawn at "
				.. minetest.pos_to_string(pos))
			else
--				print ("[mobu] Spawned " .. name .. " at "
...
function mobu:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height)

	mobu:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
...
function mobu:explosion(pos, radius, fire, smoke, sound)
...
function mobu:register_arrow(name, def)
...
-- Spawn Egg
function mobu:register_egg(mob, desc, background, addegg)

	local invimg = background

	if addegg == 1 then
		invimg = invimg .. "^mobu_simp_egg.png"
	end
...
function mobu:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
...
function mobu:feed_tame(self, clicker, feed_count, breed, tame)
